...
URLMAJORMINOR=`echo $URLVERSION | cut -d. -f1,2 | tr -d '.'`
...
if [ -f /etc/init.d/besclient ]; then
  ...
  if [ ! -f /var/opt/BESClient/besclient.config ]; then
    {
      echo "[Software\\BigFix\\EnterpriseClient]"
      echo "EnterpriseClientFolder = /opt/BESClient"
      echo
      echo "[Software\\BigFix\\EnterpriseClient\\GlobalOptions]"
      echo "StoragePath = /var/opt/BESClient"
      echo "LibPath = /opt/BESClient/BESLib"
      while IFS='=' read -r key value; do
        if [ -n "$key" ] && [ -n "$value" ]; then
          echo
          echo "[Software\\BigFix\\EnterpriseClient\\Settings\\Client\\$key]"
          echo "value = $value"
        fi
      done < /tmp/clientsettings.cfg
